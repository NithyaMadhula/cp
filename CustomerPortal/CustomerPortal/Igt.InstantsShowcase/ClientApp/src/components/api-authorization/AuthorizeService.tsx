...
export class AuthorizeService {
  ...
  _popUpDisabled = true;
    ...
    async isAuthenticated() {
        ...
        if (user) {
            await fetch_data_1.fetch_data.getUser().then(function (currentUser : any) {
                var organizationCode = currentUser.organizationCode;
  ...
  async getUser() {
    ...
    return user && { ...user.profile, access_token: user.access_token };
  }
  ...
  async signIn(
    state: any
  ): Promise<{ state?: any; status?: any; message?: string }> {
    await this.ensureUserManagerInitialized();
    try {
      const silentUser = await this.userManager.signinSilent(
        this.createArguments()
        );
      localStorage.setItem("user", JSON.stringify(silentUser));
  ...
  async completeSignIn(
    url: string
  ): Promise<{ status: any; state?: any; message?: string }> {
    try {
      await this.ensureUserManagerInitialized();
  ...
  async signOut(state: any): Promise<{ status: string; message?: string }> {
  ...
  async completeSignOut(
    url: string
  ): Promise<{ status: string; message: string; state?: any }> {
    await this.ensureUserManagerInitialized();
    try {
      const response = await this.userManager.signoutCallback(url);
      this.updateState(null);
      return { ...this.success(response && response.data), message: "" };
    } catch (error) {
  ...
  unsubscribe(subscriptionId: any) {
    ...
    if (subscriptionIndex && subscriptionIndex.length) {
