...
const { fetchNaloLotterySales, fetchNaloYtdSales, ticketBreakdown } = fetch_data;
...
const NorthAmericanLottery = (props) => {
  const [init, setInit] = useState(false);
  const [naloSalesData, setNaloSalesData] = useState({});
  const [naloYtdData, setNaloYtdData] = useState({});
  const [ticketData, setTicketData] = useState(null);
  const [start, setStart] = useState('1/1/2020');
  ...
  useEffect(() => {
    ...
    ticketBreakdown()
      .then((response) => {
        ...
        const filtered = response.filter((x) => x.percentChangeYear);
  ...
  return (
    <main>
      ...
      {!hasData(naloSalesData) || !(naloYtdData && naloYtdData.currWeek) || !hasData(ticketData) ? <LinearProgress /> : <></>}

      {naloYtdData && naloYtdData.currWeek ? (
        <>
          <h2>{props.lotteryName} Instants YTD Sales</h2>
          <ExpansionPanel>
              ...
              <Card style={{ minWidth: '700px' }}>
                <TableContainer>
                  <Table>
                    ...
                    <TableBody>
                      <TableRow>
                        <TableCell align="center">{new Date(naloYtdData.currWeek).toLocaleDateString()}</TableCell>
                        <TableCell align="center">
                          {currency_conversion.currencyFormat(naloYtdData.priorYearYTDSales, 0, true)}
                        </TableCell>
                        <TableCell align="center">{currency_conversion.currencyFormat(naloYtdData.ytdSales, 0, true)}</TableCell>
                        <TableCell align="center" style={getStyle(naloYtdData.percentChangeYear)}>
                  ...
                  {hasData(ticketData) ? (
                    <Table>
                      ...
                      <TableBody>
                        {ticketData.map((x) => (
                          <TableRow>
                            <TableCell align="center">{currency_conversion.currencyFormat(x.ticketPrice)}</TableCell>
                            ...
                            <TableCell align="right">
                              {currency_conversion.currencyFormat(x.priorYearWeekSales, 0, true)}
                            </TableCell>
                            <TableCell align="right">
                              {currency_conversion.currencyFormat(x.priorYearYTDSales, 0, true)}
                            </TableCell>
      ...
      <br /> */}
      {hasData(naloSalesData) ? <NorthAmericanLotteryTable data={naloSalesData}></NorthAmericanLotteryTable> : <></>}
    </main>
  );
};
