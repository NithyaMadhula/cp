...
{
    // TODO: Fix the original ClientStore on IdentityServer3.Dapper NuGet package
    class CustomClientStore : BaseStore, IClientStore
    {
        public CustomClientStore(DapperServiceOptions options) : base(options)
        {
        }
            ...
            using (var con = Options.OpenConnection())
            {
                var baseClient =
                    await con.QueryFirstOrDefaultAsync<BaseClient>("SELECT * FROM Clients WHERE ClientId=@Id",
                        new { Id = clientId });
                // Fix! Without this, an invalid client will throw an unhandled exception!
                if (baseClient == null) return null;
                var sql =
                @"
